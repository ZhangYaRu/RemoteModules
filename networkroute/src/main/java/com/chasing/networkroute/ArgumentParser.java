    ...
    for (int i = 0 ; i < args.length ; i++) {
      ...
      if (arg.startsWith("--")) {
        ...
        if (arg.equals("--udp-tun")) {
          if (RouteConfig.isDebug)
            Log.d("网络转发测试","parseArgs   --udp-tun");
...
//          System.out.println(help());
  ...
  private static String parseParams(List<String> paramStrs, Params params, String errors) {
    if (RouteConfig.isDebug)
      Log.d("网络转发测试","parseParams ===="+errors);

    if (paramStrs.size() == 4) {
      try {
        int sourcePort = Integer.parseInt(paramStrs.get(0));
        params.setLocalRecievePort(sourcePort);
        if (sourcePort < 1 || sourcePort > 65535)
          errors += "Port numbers have to be in range 1-65535, source port was: " + sourcePort + "." + ln;
      } catch (NumberFormatException e) {
        errors += "Unable to parse source port from: '" + paramStrs.get(0) + "'." + ln;
      }
      try {
        int localSendport = Integer.parseInt(paramStrs.get(1));
        params.setLocalSendPort(localSendport);
        if (localSendport < 1 || localSendport > 65535)
          errors += "Port numbers have to be in range 1-65535, source port was: " + localSendport + "." + ln;
      } catch (NumberFormatException e) {
        errors += "Unable to parse source port from: '" + paramStrs.get(1) + "'." + ln;
      }
      params.setRemoteHost(paramStrs.get(2));
      try {
        int remotePort = Integer.parseInt(paramStrs.get(3));
        params.setRemotePort(remotePort);
        if (remotePort < 1 || remotePort > 65535)
          errors += "Port numbers have to be in range 1-65535, remote port was: " + remotePort + "." + ln;
      } catch (NumberFormatException e) {
        errors += "Unable to parse remote port from: '" + paramStrs.get(2) + "'." + ln;
      }
    } else if(paramStrs.size() == 3) {
      try {
        int sourcePort = Integer.parseInt(paramStrs.get(0));
        params.setLocalRecievePort(sourcePort);
